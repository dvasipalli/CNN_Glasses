...
def convert_all_imgs_to_idx3(dset, name):
    # Load from and save to
    dset = os.path.join('Dataset', dset)
    FileList = []
    
    for di in os.listdir(dset):
        path = os.path.join(dset, di)
        for fi in os.listdir(path):
            if fi.endswith(".pgm"):
                FileList.append(os.path.join(path, fi))
                    
    save_imgs_labes_to_idx3(FileList, name)

def save_imgs_labes_to_idx3(FileList, name):
    shuffle(FileList) # Usefull for further segmenting the validation set  width = 0
    height = 0
    data_image = array('B')
    data_label = array('B')
    ...
    output_file = open('Dataset/'+name+'-images-idx3-ubyte', 'wb')
    data_image.tofile(output_file)
    output_file.close()
    
    output_file = open('Dataset/'+name+'-labels-idx1-ubyte', 'wb')
